...
pub fn create_winit_window_attributes(
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Desktop launchers on X11 provide DESKTOP_STARTUP_ID, while launchers
        // in a Wayland session normally provide XDG_ACTIVATION_TOKEN. AHE uses
        // XWayland on Wayland for reliable file drops, so accept either token
        // and attach it to the X11 window. KWin can then associate the surface
        // with the launch context and choose the correct output.
        let activation_token = std::env::var("DESKTOP_STARTUP_ID")
            .or_else(|_| std::env::var("XDG_ACTIVATION_TOKEN"))
            .ok();
        if let Some(activation_token) = activation_token {
            window_attributes = window_attributes
                .with_activation_token(ActivationToken::from_raw(activation_token));
